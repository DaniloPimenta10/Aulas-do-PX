...

{
    ListNode *node = (ListNode *)malloc(sizeof(ListNode)); // Allocate memory for a new node.
    node->info = v;                                        // Set the 'info' field of the new node to the provided value.
    node->prev = l->last;                                 // Make the new node point to the current lat node.
    node->next = NULL;

    if (!ll_is_empty(l))
        l->last->next = node; // Set the previous of the current last node to the new node.
    else
        l->first = node; // If the list was empty, the new node is also the first node.
